...
const SITO_ROUTES = [
  { to: '/app/site/dashboard', label: 'Dashboard', icon: Monitor },
  { to: '/app/site/contents', label: 'Contenuti', icon: FileText },
];
...
export function Sidebar() {
  ...
  const initialOpen = useMemo(() => SITO_ROUTES.some(route => pathname?.startsWith(route.to)), [pathname]);
        ...
        <div className="space-y-2">
          <button
            ...
            className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition hover:text-neutral-900 ${
              SITO_ROUTES.some(route => pathname?.startsWith(route.to))
                ? 'text-neutral-900'
                : 'text-neutral-600'
            }`}
          >
